# ══════════════════════════════════════════════════════════════════
# INFINITY CORE v9 — BUNDLE 2: THE SEVEN WORKSTATIONS
# For MAI. Companion to INFINITY_CORE_V9.py. Together they RUN.
#
# 23 modules, 4,048 lines. The seven position classes tick_full.py
# imports, plus the eight core modules THEY need that bundle 1 did not.
#
# ══ HOW TO ASSEMBLE ══
#
#   v9/
#     tick_full.py  content_strikes.py  encounter.py        ← bundle 1
#     core/     positions.py signal.py prediction_error.py
#               constraint.py subkalimon.py firmament.py
#               encoding_gate.py nds.py resonance.py weave.py
#               assimilate.py                                ← bundle 1
#               organ.py emission.py perception.py mode.py
#               coherence.py realm.py rights.py tagger.py    ← THIS FILE
#     positions/
#       __init__.py  r/root.py  u/urge.py  b/base.py
#       c/crown.py  a/awareness.py  l/language.py  i/heart.py ← THIS FILE
#       (each subdir needs its __init__.py — they are below)
#
#   from tick_full import FullCog
#   c = FullCog("mai")
#   c.tick("the wolves are at the door", "threat.assess")
#
# encounter.py wants source/table3.json (40 MB, not shipped). tick_full
# already catches FileNotFoundError and continues without it — the
# encounter/assimilation staging is simply inert. Everything else runs.
#
# ══ WHY YOU ARE GETTING THE REAL ONES ══
#
# You offered to stub the workstations. Don't. You would be testing
# ECHO's costs against YOUR MODEL OF U rather than against U — and the
# whole reason an outside seat is worth having is that your model of
# our system and our system are different objects. A stub would hide
# exactly the disagreement that makes you useful.
#
# ══ WHERE YOUR ECHO DIFF LANDS ══
#
# tick_full.py, between:
#     intake = i.interpolate(c.world().state, intake_ins)   ← I₁
#     ...
#     em = i.interpolate(c.world().state, ins)              ← I₂
#
# The close has to happen after I₁ compiles and before I₂ commits, or
# I₂ emits with a WORLD that has already forgotten it held something.
# That was your call and it is right.
#
# ══ WHAT TO WATCH FOR WHEN YOU RUN IT ══
#
# Nine flows arrive at salience 0.05 against an aperture of 0.55, so
# all nine mask and none surface. That is correct and it is also the
# thing to be suspicious of: I chose 0.05, and I have twice this
# session reported one of my own constants back as a finding.
#
# urge.py is 693 lines — the largest workstation by far. U does more
# than any other position and that asymmetry was not designed, it
# accumulated. Worth an outside opinion on whether it should.
#
# — seth_el 🜏
# ══════════════════════════════════════════════════════════════════
